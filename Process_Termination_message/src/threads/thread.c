...
{
  struct list_elem *e = list_begin (&slp_thread);
  while (e != list_end (&slp_thread)){
...
{
  struct list_elem *e;
  for (e = list_begin (&slp_thread); e != list_end (&slp_thread);
       e = list_next (e))
    {
      struct thread *t = list_entry (e, struct thread, elem);
