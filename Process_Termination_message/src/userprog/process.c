...
pid_t process_execute (const char *file_name)
{
  ...
  struct thread *thr = thread_get(tid);
  thr->next_fd = 2;
  thr->prog_name = cmd_name;
  list_init(&thr->desc_table);
  list_init(&thr->children);
...
static bool install_page (void *upage, void *kpage, bool writable)
{
  ...
  return (pagedir_get_page (thr->pagedir, upage) == NULL
          && pagedir_set_page (thr->pagedir, upage, kpage, writable));
}
...
void process_close (int fd)
{
  if (get_fd_entry(fd) != NULL){
    struct fd_entry *fd_en = get_fd_entry(fd);
    file_close(fd_en->file);
    list_remove(&fd_en->elem);
    free(fd_en);
  }
}
...
{
  struct list *fd_table = &thread_current()->desc_table;
  struct list_elem *el = list_begin (fd_table);
  while (el != list_end (fd_table))
    {
      struct fd_entry *tmp = list_entry (el, struct fd_entry, elem);
      el = list_next (el);
